...
#ifndef BV3C2_H_INCLUDED
#define BV3C2_H_INCLUDED
...
namespace HYDROTEL
{
	...
	class BV3C2 : public BILAN_VERTICAL
	{
	public:
		BV3C2(SIM_HYD& sim_hyd);
		virtual ~BV3C2();
		...
		/// retourne la sauvegarde de tous les etats
	...
	private:
		...
		std::vector<float> _des;						//parameter	//COEFFICIENT D'EXTINCTION
		std::vector<float> _cin;
		std::vector<float> _coef_assech;

		std::vector<float> _coefRechargeSousTerrain;	//[0-1]
		...
		//somme annuelle q23
		std::vector<std::vector<float>> _q23_sum;				//mm/year		//[uhrh index][simulation year]
		int								_iQ23SumYearStart;
		int								_iQ23SumYearEnd;
		int								_iQ23SumYearCurrent;
